...
- **Mattermost leading-space slash commands** — Normalises platform-escaped commands (e.g. ` /new`) in the Mattermost adapter so mobile users can send slash commands. **Scoped to `plugins/platforms/mattermost/adapter.py` only — no `gateway/platforms/base.py` changes.** The initial attempt (`68881dc63`, adding `.strip()` to `is_command()` in the gateway base class) was superseded and reverted by the adapter fix (`fc6549f9e`). When rebasing from upstream or reviewing fork history, **do not re-apply the gateway fix** — it was intentionally walked back. The adapter-side approach is correct and self-contained.
